...
import net.frozenorb.foxtrot.map.challenges.command.ChallengePickNewCommand;
import net.frozenorb.foxtrot.map.challenges.command.ChallengeProgress;
import net.frozenorb.foxtrot.map.challenges.command.TokensCommand;
import net.frozenorb.foxtrot.map.challenges.impl.KillBasedChallenge;
import net.frozenorb.foxtrot.map.challenges.impl.KillKitBasedChallenge;
import net.frozenorb.foxtrot.map.challenges.impl.util.KitBasedChallengeData;
...
public class ChallengeHandler implements Listener {
    ...
    public ChallengeHandler() {
        ...
        ClassUtils.getClassesInPackage(Foxtrot.getInstance(), "net.frozenorb.foxtrot.map.challenges.impl").forEach(clazz -> {
        	// Do not attempt to register if the class is not a challenge
        	if (!Challenge.class.isAssignableFrom(clazz)) {
        		return;
	        }

        	// Do not attempt to register if the class is an interface or is abstract
        	if (clazz.isInterface() || !Modifier.isAbstract(clazz.getModifiers())) {
        		return;
	        }

            // Do not attempt to register if class should be programmatically loaded
            if (clazz.getAnnotation(ProgrammaticallyLoaded.class) != null) {
                return;
            }
