...
public class FakeWallThread extends Thread {

	private final NightmareHandler handler;
	private final Map<String, Map<Location, Long>> sentBlockChanges = new HashMap<>();

	public FakeWallThread(NightmareHandler handler) {
		super("Foxtrot - Fake Wall Thread");
	...
	private void checkPlayer(Player player) {
		try {
			...
			List<Location> blockUpdateLocations = new LinkedList<>();

			ProgressData progressData = handler.getOrCreateProgression(player.getUniqueId());

			for (Map.Entry<Integer, List<Location>> entry : handler.getWallLocations().entrySet()) {
				Integer wallStage = entry.getKey();
				List<Location> wallLocations = entry.getValue();

				if (progressData.getStagesComplete() >= wallStage) {
					continue;
				}

				blockUpdateLocations.addAll(wallLocations);
			}
	...
	private void sendUpdateToPlayer(Player player, Location location) {
		...
		if (location.getWorld().isChunkLoaded(location.getBlockX() >> 4, location.getBlockZ() >> 4) && location.distanceSquared(location) < 64) {
			...
			sentBlockChanges.get(player.getName()).put(location, System.currentTimeMillis() + 1500L);
		}
	}
